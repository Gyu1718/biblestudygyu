(function () {
  ...
  var aliasPattern = Object.keys(aliasToBook)
  ...
  function ensureUI() {
    ...
    panel.innerHTML = [
      ...
      '<div><div class="br-eyebrow">BIBLE READER</div><h2>성경 검색기</h2></div>',
      '<button class="br-close" type="button" aria-label="성경 패널 닫기">×</button>',
      '</header>',
      '<form class="br-search"><input type="search" placeholder="예: 학개 1:5–6" aria-label="성경 장절 검색"><button type="submit">열기</button></form>',
      '<div class="br-status" aria-live="polite"></div>',
      '<div class="br-content"><p class="br-empty">연구 페이지의 성경 장절을 누르거나 위 검색창에 장절을 입력하세요.</p></div>',
      '<footer class="br-foot">개역개정 본문 · 학업·학술용 허가 범위 내 사용</footer>'
    ].join("");
    ...
    panel.querySelector(".br-search").addEventListener("submit", function (event) {
  ...
  function setStatus(message) {
    ensureUI().panel.querySelector(".br-status").textContent = message || "";
  }

  function loadBook(code) {
    if (cache.has(code)) return cache.get(code);
    var promise = fetch(DATA_ROOT + code + ".json", { credentials: "same-origin" })
      .then(function (response) {
        if (!response.ok) throw new Error("본문 데이터 없음");
  ...
  function getVerses(data, ref, limit) {
    ...
    var end = ref.verse ? (ref.endVerse || ref.verse) : Math.max.apply(null, Object.keys(chapter).map(Number));
    var rows = [];
  ...
  function openReference(ref, updateHash) {
    ...
    loadBook(ref.code).then(function (data) {
      ...
      setStatus("");
      view.panel.querySelector(".br-content").innerHTML = '<div class="br-ref-title"><span>' + escapeHtml(data.translation) + '</span><h3>' + escapeHtml(labelFor(ref)) + '</h3></div>' + renderVerses(rows);
      if (updateHash !== false) history.replaceState(null, "", "#bible=" + encodeURIComponent(refKey(ref)));
    }).catch(function (error) {
      setStatus("");
      view.panel.querySelector(".br-content").innerHTML = '<div class="br-error"><b>' + escapeHtml(labelFor(ref)) + '</b><p>' + escapeHtml(error.message) + '</p><small>현재 느헤미야·호세아·학개 본문부터 연결되어 있습니다.</small></div>';
    });
  }
  ...
  function bindReferences() {
    ...
    document.addEventListener("mouseover", function (event) {
      ...
      clearTimeout(hoverTimer);
      var ref = parseKey(anchor.dataset.bibleRef);
      if (ref) hoverTimer = setTimeout(function () { showTooltip(anchor, ref); }, 280);
    });
    document.addEventListener("mouseout", function (event) {
      if (event.target.closest(".scripture-ref")) hideTooltip();
    });
