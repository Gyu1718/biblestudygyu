...
- 공통 연구 도크의 실제 버튼 위치를 측정해 `편 찾기`를 그 위에 자동 배치합니다.
- 데스크톱·모바일·가로 화면과 화면 크기 변경에 맞춰 좌표를 다시 계산합니다.
- 연구 도크가 열리면 `편 찾기` 버튼과 팝오버를 잠시 숨깁니다.
- `편 찾기`의 z-index를 연구 도크보다 낮춰 화면 설정 버튼을 가리지 않습니다.
